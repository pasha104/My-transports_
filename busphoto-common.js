const defaultVehicleDB = [
        ...
        function refreshOwnedGameVehicles() {
            const el = document.getElementById('ownedGameVehicles');
            if (!el) return;
            let state = null;
            try {
            ...
            if (!owned.length) {
                el.innerHTML = '<div style="padding:10px;border:1px dashed var(--bp-border);opacity:.75;font-size:11px;">Покупных ТС пока нет. Купи транспорт в разделе «Интерактив → Магазин».</div>';
                return;
            }
            el.innerHTML = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:7px;">' + owned.map((v,i) => {
                const icon = v.category === 'trolleybus' ? '🚎' : v.category === 'electrobus' ? '⚡' : '🚌';
                return `<div style="padding:9px;border:1px solid var(--bp-border);border-radius:6px;background:var(--bp-input-bg);font-size:11px;">
                    <b>${icon} ${escapeHtml(v.model || 'ТС')}</b><br>
                    Госномер: ${escapeHtml(v.plate || '—')}<br>
                    Борт. №: ${escapeHtml(v.num || '—')}<br>
                    Стоимость: ${Number(v.price||0).toLocaleString('ru-RU')} ₽
                </div>`;
            }).join('') + '</div>';
        }
