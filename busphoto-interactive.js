/* ===== Интерактив: транспортный бизнес ===== */
        ...
        function countTerminalArrivalsBetween(card, route, pair, fromMs, toMs) {
            ...
            const firstDrive = Math.max(60, Number(route.calculatedDuration || 1200)) / 60;
            const secondDrive = Math.max(60, Number(pair?.calculatedDuration || route.calculatedDuration || 1200)) / 60;
...
`;
        ...
        function updateMapBusMarkers(forceVehicleMode = false) {
            ...
            gameState.serviceCards.forEach(card => {
                ...
                const key = String(card.vehicleId);
                if (!cardsByVehicle.has(key)) cardsByVehicle.set(key, []);
                cardsByVehicle.get(key).push(card);
            });
            const assignedByCard = new Set(cardsByVehicle.keys());
            const nowMinutes = getCurrentMinutesOfDay();

            vehicles.forEach(vehicle => {
                const cards = cardsByVehicle.get(String(vehicle.id)) || [];
                let active = null;
                let bestStart = -Infinity;
                cards.forEach(card => {
                    const win = cardTimeWindow(card);
                ...
                if (active) {
                    ...
                    if (route) {
                        ...
                        // Финансовые начисления обрабатываются отдельным лёгким таймером,
                        // а не на каждом обновлении карты. Это резко снижает нагрузку на телефон.
        ...
        function startMapBusAnimation() {
            ...
            const loop = () => {
                ...
                // 4–5 обновлений/с достаточно для игровой карты и заметно легче для телефона.
                updateMapBusMarkers(true);
                mapState.busAnimationFrame = setTimeout(loop, 220);
            };
        ...
        function renderInteractive() {
            ...
            const routesSection = document.getElementById('game-section-routes');
            if (routesSection?.classList.contains('active')) renderRoutes();
        }
        ...
        function initInteractiveGame() {
            ...
            if (!window.gameMinuteTimer) {
                // Проверяем начисления раз в 15 секунд вместо полного перерендера каждые 5 с.
                // Это заметно снижает нагрузку на слабых телефонах; начисление считается
                // по реальному времени, поэтому пропущенные интервалы не теряются.
                window.gameMinuteTimer = setInterval(() => {
                    const earned = processAllOfflineEarnings();
                    if (earned) renderInteractiveHeaderAndLightViews();
                }, 15000);
            }
        }
