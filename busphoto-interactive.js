        ...
        function loadGameState() {
            ...
            gameState.serviceCards.forEach(card => {
                ...
                if (!Array.isArray(card.schedules) || !card.schedules.length) {
                    card.schedules = [{days:[0,1,2,3,4,5,6], depotTime:card.depotTime || '07:00', startTime:card.startTime || '07:30', endTime:card.endTime || '08:30', returnTime:card.returnTime || '09:00'}];
                }
...
`;
        ...
        function updateMapBusMarkers(forceVehicleMode = false) {
            ...
            vehicles.forEach(vehicle => {
                ...
                const activeSchedule = serviceCard ? getActiveDepartureSchedule(serviceCard, new Date(nowMs)) : null;
                if (serviceCard && !activeSchedule) return;
                const hm = t => { const [h,m] = String(t || '00:00').split(':').map(Number); return h*60+m; };
                const scheduleStart = hm(activeSchedule.schedule.startTime);
                const beforeRoute = serviceCard && activeSchedule.nowMinutes < scheduleStart;
                const elapsed = serviceCard
                    ? Math.max(0, (activeSchedule.nowMinutes - scheduleStart) * 60000)
                    : nowMs;
                const movementElapsed = elapsed + getVehiclePhaseOffset(vehicle.id, route.id, cycleDuration);
                const t = (movementElapsed % cycleDuration) / cycleDuration;
                let point = null;
                let phase = '';
                let directionLabel = '→';
                if (beforeRoute) {
                    point = [depot.lat, depot.lon];
                    phase = `выезд из парка · до ${activeSchedule.schedule.startTime}`;
                } else if (t < 0.5) {
                ...
                if (serviceCard && activeSchedule) {
                    const outboundArrivalIndex = Math.floor(elapsed / duration);
                    const stamp = `${localDateKey(new Date(nowMs))}|${activeSchedule.index}|${outboundArrivalIndex}`;
                    if (outboundArrivalIndex > 0 && serviceCard.lastPayoutStamp !== stamp) {
                        gameState.balance += 100;
                        serviceCard.lastPayoutStamp = stamp;
                        saveGameState();
        ...
        function getVehicleSimulationPoint(vehicle) {
            const serviceCard = getVehicleServiceCard(vehicle.id);
            const route = serviceCard ? gameState.routes.find(r => String(r.id) === String(serviceCard.routeId)) : getVehicleRoute(vehicle.id);
            ...
            const duration = Math.max(60000, Number(route.calculatedDuration || 720) * 1000);
            const cycleDuration = duration * 2;
            const activeSchedule = serviceCard ? getActiveDepartureSchedule(serviceCard, new Date()) : null;
            if (serviceCard && !activeSchedule) return {route, point:null, phase:'в парке — по расписанию сейчас нет выезда'};
            const hm = t => { const [h,m] = String(t || '00:00').split(':').map(Number); return h*60+m; };
            const scheduleStart = hm(activeSchedule.schedule.startTime);
            const beforeRoute = serviceCard && activeSchedule.nowMinutes < scheduleStart;
            const elapsed = serviceCard ? Math.max(0, (activeSchedule.nowMinutes - scheduleStart) * 60000) : Date.now();
            const phaseOffset = getVehiclePhaseOffset(vehicle.id, route.id, cycleDuration);
            const t = ((elapsed + phaseOffset) % cycleDuration) / cycleDuration;
            let point, phase;
            if (beforeRoute) { point = [DEPOTS[vehicle.category]?.lat || 53.9, DEPOTS[vehicle.category]?.lon || 27.56]; phase = `выезд из парка · до ${activeSchedule.schedule.startTime}`; }
            else if (t < 0.5) { point = pathPointFromGeometry(route.geometry, t*2); phase = `на маршруте №${route.number} →`; }
            else { const returnGeometry = pair?.geometry || route.returnGeometry || route.geometry; point = pathPointFromGeometry(returnGeometry, (t-0.5)*2); phase = `на маршруте №${pair?.number || route.number} ←`; }
            return {route, point, phase};
        }
        ...
        function updateDepartureRouteInfo() {
            ...
            if (!el) return;
            if (!route) { el.textContent = 'Выбери маршрут — здесь появится примерное время движения.'; return; }
            const duration = Number(route.calculatedDuration || 0);
            const distance = Number(route.calculatedDistance || 0);
            const bits = [];
            if (duration) bits.push(`примерное время в пути: ${formatMinutesToDuration(duration)}`);
            if (distance) bits.push(`${(distance / 1000).toFixed(1)} км`);
            el.textContent = bits.length ? 'ℹ️ ' + bits.join(' · ') : 'ℹ️ Для маршрута пока нет расчёта времени движения.';
        }
        ...
        function addDepartureScheduleRow(prefill) {
            ...
            if (!wrap) return;
            const s = prefill || {day:'1', depotTime:'07:00', startTime:'07:30', endTime:'08:30', returnTime:'09:00'};
            const row = document.createElement('div');
            row.className = 'departure-schedule-row';
            row.innerHTML = `
                ...
                <input class="departure-depot" type="time" value="${escapeHtml(s.depotTime)}" required title="Выезд из парка">
                <input class="departure-start" type="time" value="${escapeHtml(s.startTime)}" required title="Отправление с первой конечной">
                <input class="departure-end" type="time" value="${escapeHtml(s.endTime)}" required title="Прибытие на конечную">
                <input class="departure-return" type="time" value="${escapeHtml(s.returnTime)}" required title="Заезд в парк">
        ...
        function collectDepartureSchedules() {
            ...
            rows.forEach(row => {
                ...
                const schedule = {
                    depotTime: row.querySelector('.departure-depot')?.value || '07:00',
                    startTime: row.querySelector('.departure-start')?.value || '07:30',
                    endTime: row.querySelector('.departure-end')?.value || '08:30',
                    returnTime: row.querySelector('.departure-return')?.value || '09:00'
                };
        ...
        function renderDepartureCards() {
            ...
            el.innerHTML = cards.map(card => {
                ...
                const rows = schedules.map(s => (s.days || []).map(day => `<div class="departure-schedule-view"><b>${DAY_FULL_RU[Number(day)] || 'День'}</b><span>🚍 ${escapeHtml(s.depotTime)}</span><span>▶️ ${escapeHtml(s.startTime)}</span><span>🏁 ${escapeHtml(s.endTime)}</span><span>🏠 ${escapeHtml(s.returnTime)}</span></div>`).join('')).join('');
                const duration = Number(r.calculatedDuration || 0) ? formatMinutesToDuration(r.calculatedDuration) : 'не рассчитано';
                return `<div class="departure-card"><div class="departure-card-head"><div><b>🗓️ Выезд · №${escapeHtml(r.number)}</b><div class="interactive-muted">${vehicleCategoryIcon(v.category)} ${escapeHtml(v.model)} · ${escapeHtml(v.category === 'trolleybus' ? ('борт. №' + v.num) : (v.plate || v.num || 'без номера'))}</div><div class="interactive-muted">${escapeHtml(r.start)} → ${escapeHtml(r.end)} · ⏱️ ${duration}</div></div><button class="btn-secondary" onclick="toggleDepartureCard('${escapeHtml(card.id)}')">${card.active !== false ? '⏸ Остановить' : '▶ Запустить'}</button><button class="btn-secondary" onclick="deleteDepartureCard('${escapeHtml(card.id)}')">Удалить</button></div><div class="departure-schedule-list">${rows || '<div class="interactive-muted">Расписание не задано.</div>'}</div></div>`;
        ...
        function saveDepartureCard(event) {
            ...
            if (!schedules.length) return alert('Добавь хотя бы один выезд с днём недели.');
            ...
            gameState.serviceCards.unshift({id,vehicleId:vehicle.id,routeId:route.id,schedules,createdAt:new Date().toISOString(),lastArrivalCount:0,lastPayoutStamp:'',active:true});
            saveGameState(); renderDepartureCards(); document.getElementById('departureCardForm').reset();
            const rows = document.getElementById('departureScheduleRows'); if (rows) rows.innerHTML = '';
            addDepartureScheduleRow();
            updateDepartureRouteOptions();
            updateMapBusMarkers(true);
        }
        ...
        function getActiveDepartureSchedule(card, now = new Date()) {
            ...
            for (let i = 0; i < schedules.length; i++) {
                ...
                const hm = t => { const [h,m] = String(t || '00:00').split(':').map(Number); return h*60+m; };
                const depot = hm(s.depotTime), ret = hm(s.returnTime);
        ...
        function toggleDepartureCard(id) { const card = gameState.serviceCards.find(c => String(c.id) === String(id)); if (!card) return; card.active = card.active === false; if (card.active) { card.createdAt = new Date().toISOString(); card.lastPayoutStamp = ''; } saveGameState(); renderDepartureCards(); updateMapBusMarkers(true); }
