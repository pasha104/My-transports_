/* ===== Интерактив: транспортный бизнес ===== */
        ...
        const GAME_BUS_CLASSES = {
            ...
            'МАЗ-215':['city','suburban'],
            'МАЗ-216':['city','suburban'],
            'МАЗ-226':['city','suburban'],
            'МАЗ-231':['city','suburban'],
            'МАЗ-232':['city','suburban'],
            'МАЗ-241':['city','suburban'],
            'МАЗ-251':['intercity'],
            'МАЗ-256':['suburban','intercity'],
            'МАЗ-257':['suburban','intercity'],
            'МАЗ-303':['city','suburban']
        };
        const GAME_BUS_SUBMODELS = {
            'МАЗ-101':['101'],
            'МАЗ-103':['103.000','103.002','103.003','103.005','103.040','103.041','103.060','103.061','103.062','103.065','103.067','103.068','103.070','103.075','103.076','103.464','103.465','103.468','103.469','103.476','103.485','103.486','103.562','103.564','103.565','103.569','103.585','103.586','103.587','103С00','103С02','103С03','103С05','103С40','103С60','103С61','103С62','103С65','103С67','103С70','103С76'],
            ...
            'МАЗ-226':['226.047','226.068','226.100'],
            ...
            'МАЗ-303':['303.047','303.065','303.066','303.226','303.266','303.147','303е10','303е20','303е22']
        };
...
`;
        ...
        function getScheduleVehiclePoint(schedule, route, vehicle, now = new Date()) {
            ...
            const forwardGeometry = forwardFirst ? route.geometry : reverseGeometry(route.geometry);
            const reverseRouteGeometry = forwardFirst ? reverseGeometry(route.geometry) : route.geometry;
            const outboundGeometry = forwardFirst ? route.outboundGeometry : reverseGeometry(route.returnGeometry);
            const returnGeometry = forwardFirst ? route.returnGeometry : reverseGeometry(route.outboundGeometry);
            ...
            if (currentMs <= lastArrivalMs) {
                ...
                return {route,point:pathPointFromGeometry(isForward ? forwardGeometry : reverseRouteGeometry,progress),phase:`на маршруте №${route.number} ${isForward?'→':'←'}`};
            }
        ...
        function addDepartureScheduleRow(s = {}) {
            ...
            row.innerHTML = `<select class="departure-day" aria-label="Дни недели"><option value="weekdays">Будни</option><option value="weekends">Выходные</option><option value="all">Все дни</option><option value="1">Понедельник</option><option value="2">Вторник</option><option value="3">Среда</option><option value="4">Четверг</option><option value="5">Пятница</option><option value="6">Суббота</option><option value="0">Воскресенье</option></select>
            ...
            row.querySelector('.departure-depot').addEventListener('change', () => autoCalculateDepartureRow(row));
            row.querySelector('.departure-until').addEventListener('change', () => autoCalculateDepartureRow(row));
            updateDepartureRouteForRows();
        ...
        function getActiveDepartureSchedule(card, now = new Date()) {
            ...
            for (let i=0;i<schedules.length;i++) {
                ...
                for (let offset=0;offset<=1;offset++) {
                    ...
                    if(nowMs>=abs.depot.getTime() && nowMs<=abs.return.getTime()) return {schedule:s,index:i,startMinutes:abs.startMin,nowMinutes:nowMs/60000,absolute:abs,baseDate:base};
                }
            }
            return null;
        }
