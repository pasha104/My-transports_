...
`;
        ...
        function findReverseRoute(route) {
            ...
            const same = gameState.routes.filter(r =>
                ...
                String(r.number || '') === String(route.number || '') &&
                r.geometry &&
        ...
        function getScheduleVehiclePoint(schedule, route, vehicle, now = new Date()) {
            ...
            const outboundGeometry = route.outboundGeometry;
            const returnGeometry = route.returnGeometry;
        ...
        function addDepartureScheduleRow(s = {}) {
            ...
            row.innerHTML = `<select class="departure-day" aria-label="Дни недели"><option value="weekdays">Будни</option><option value="weekends">Выходные</option><option value="all">Все дни</option><option value="1">Понедельник</option><option value="2">Вторник</option><option value="3">Среда</option><option value="4">Четверг</option><option value="5">Пятница</option><option value="6">Суббота</option><option value="0">Воскресенье</option></select>
                ...
                <button type="button" class="btn-secondary departure-remove-row" onclick="this.closest('.departure-schedule-row').remove(); updateDepartureCardBuilderSummary();" title="Удалить этот выезд">🗑️</button>
        ...
        function collectDepartureSchedules() {
            ...
            document.querySelectorAll('#departureScheduleRows .departure-schedule-row').forEach(row => {
                ...
                const schedule = {
                    ...
                    estimatedTrips: getDepartureRowTripCount(row),
