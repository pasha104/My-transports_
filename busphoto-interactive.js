/* ===== Интерактив: транспортный бизнес ===== */
        ...
        /* v14: вся Беларусь — 6 областей + 118 районов. Загружаются по одному региону и кэшируются. */
        ...
        function initStopRegionSelect() {
            ...
            const oblastOrder = ['Минск','Брестская область','Витебская область','Гомельская область','Гродненская область','Минская область','Могилёвская область'];
...
`;
        ...
        function getVehicleSimulationPoint(vehicle) {
            ...
            const duration = Math.max(60000, Number(route.calculatedDuration || 720) * 1000);
            const cycleDuration = duration * 2;
            const elapsed = serviceCard ? Math.max(0, Date.now() - new Date(serviceCard.createdAt || Date.now()).getTime()) : Date.now();
            ...
            let point, phase;
            if (t < 0.5) { point = pathPointFromGeometry(route.geometry, t*2); phase = `на маршруте №${route.number} →`; }
