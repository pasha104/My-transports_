/* ===== Интерактив: транспортный бизнес ===== */
        ...
        /* v15: Беларусь + выбранные регионы РФ; расписание карточек выезда; 4-значные бортовые номера троллейбусов. */
        ...
        function loadGameState() {
            ...
            gameState.serviceCards.forEach(card => {
                ...
                card.schedules = card.schedules.map(s => ({
                    ...
                    depotTime: s.depotTime || card.depotTime || '07:00',
                    startTime: s.startTime || card.startTime || '07:30',
                    endTime: s.endTime || card.endTime || '08:30',
                    returnTime: s.returnTime || card.returnTime || '09:00'
                }));
        ...
        function selectGameCategory(category, btn) {
            ...
            document.querySelectorAll('#gameCategoryPicker .game-category-btn').forEach(b => {
...
`;
        ...
        function processServiceCardPayouts(now = new Date()) {
            ...
            (gameState.serviceCards || []).forEach(card => {
                ...
                const lastStamp = String(card.lastArrivalStamp || '');
                let newestStamp = lastStamp;
                for (let d = new Date(cursor); d <= now; d.setDate(d.getDate()+1)) {
                    const day = d.getDay();
                    (card.schedules || []).forEach((s, si) => {
                        if (!Array.isArray(s.days) || !s.days.includes(day)) return;
                        const arrival = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 0, 0, 0, 0);
                        const [h,m] = String(s.endTime || '00:00').split(':').map(Number);
                        arrival.setHours(Number(h)||0, Number(m)||0, 0, 0);
                        if (arrival <= from || arrival > now) return;
                        const stamp = `${localDateKey(arrival)}|${si}|${String(s.routeId)}`;
                        if (stamp === lastStamp) return;
                        total += 100;
                        changed = true;
                        newestStamp = stamp;
                        card.lastArrivalCount = Number(card.lastArrivalCount || 0) + 1;
                        gameState.log.unshift({date:localDateKey(arrival), total:100, details:[`Прибытие на конечную маршрута №${getScheduleRoute(s)?.number || '—'}: +100 р.`]});
                    });
                }
                card.lastArrivalStamp = newestStamp;
            ...
            if (changed) {
                ...
                // Сохраняем момент проверки даже если начислений не было — это важно для офлайн-режима.
                saveGameState();
            }
            return total;
        }
        ...
        function updateMapBusMarkers(forceVehicleMode = false) {
            ...
            vehicles.forEach(vehicle => {
                ...
                const startMinutes = schedule ? timeToMinutes(schedule.startTime) : 0;
                const depotMinutes = schedule ? timeToMinutes(schedule.depotTime) : 0;
                const endMinutes = schedule ? timeToMinutes(schedule.endTime) : startMinutes + getScheduleDurationMinutes(schedule);
                const returnMinutes = schedule ? timeToMinutes(schedule.returnTime) : endMinutes + 15;
                const nowMinutes = now.getHours()*60 + now.getMinutes() + now.getSeconds()/60;
                const forward = !schedule?.startStopId || String(schedule.startStopId) === String((route.terminalStopIds || route.stopIds || [])[0]);
                const routeGeometry = forward ? route.geometry : reverseGeometry(route.geometry);
                const outboundGeometry = forward ? route.outboundGeometry : reverseGeometry(route.returnGeometry);
                const returnGeometry = forward ? route.returnGeometry : reverseGeometry(route.outboundGeometry);
                ...
                if (!serviceCard) {
                    ...
                    if (t < 0.5) { point = pathPointFromGeometry(routeGeometry, t*2); phase = `на маршруте №${route.number} →`; }
                    else { point = pathPointFromGeometry(routeGeometry, 1-(t-0.5)*2); phase = `на маршруте №${route.number} ←`; }
                } else if (nowMinutes < depotMinutes || nowMinutes > returnMinutes) {
                    point = [depot.lat, depot.lon];
                    phase = nowMinutes < depotMinutes ? `в парке · выезд в ${schedule.depotTime}` : 'в парке · рейс завершён';
                } else if (nowMinutes < startMinutes) {
                    const span = Math.max(1, startMinutes - depotMinutes);
                    const progress = Math.max(0, Math.min(1, (nowMinutes - depotMinutes) / span));
                    point = outboundGeometry ? pathPointFromGeometry(outboundGeometry, progress) : [depot.lat, depot.lon];
                    phase = `выезд из парка → ${route.start}`;
                } else if (nowMinutes <= endMinutes) {
                    const span = Math.max(1, endMinutes - startMinutes);
                    const progress = Math.max(0, Math.min(1, (nowMinutes - startMinutes) / span));
                    point = pathPointFromGeometry(routeGeometry, progress);
                    phase = `на маршруте №${route.number} →`;
                } else {
                    const span = Math.max(1, returnMinutes - endMinutes);
                    const progress = Math.max(0, Math.min(1, (nowMinutes - endMinutes) / span));
                    point = returnGeometry ? pathPointFromGeometry(returnGeometry, progress) : [depot.lat, depot.lon];
                    phase = `заезд в парк №${escapeHtml(depot.id || '')}`;
                }
        ...
        function getVehicleSimulationPoint(vehicle) {
            ...
            if (!serviceCard) {
                ...
                const point = t < 0.5 ? pathPointFromGeometry(route.geometry,t*2) : pathPointFromGeometry(route.geometry,1-(t-0.5)*2);
                return {route,point,phase:`на маршруте №${route.number}`};
            }
            const s = activeSchedule.schedule;
            const now = new Date();
            const nowMin = now.getHours()*60 + now.getMinutes() + now.getSeconds()/60;
            const depotMin = timeToMinutes(s.depotTime), startMin=timeToMinutes(s.startTime), endMin=timeToMinutes(s.endTime), retMin=timeToMinutes(s.returnTime);
            const forward = !s.startStopId || String(s.startStopId) === String((route.terminalStopIds || route.stopIds || [])[0]);
            const routeGeometry = forward ? route.geometry : reverseGeometry(route.geometry);
            const outboundGeometry = forward ? route.outboundGeometry : reverseGeometry(route.returnGeometry);
            const returnGeometry = forward ? route.returnGeometry : reverseGeometry(route.outboundGeometry);
            if (nowMin < depotMin || nowMin > retMin) return {route,point:null,phase:'в парке — рейс не выполняется сейчас'};
            if (nowMin < startMin) {
                const progress=Math.max(0,Math.min(1,(nowMin-depotMin)/Math.max(1,startMin-depotMin)));
                return {route,point:outboundGeometry?pathPointFromGeometry(outboundGeometry,progress):[depot.lat,depot.lon],phase:`выезд из парка → ${route.start}`};
            }
            if (nowMin <= endMin) {
                const progress=Math.max(0,Math.min(1,(nowMin-startMin)/Math.max(1,endMin-startMin)));
                return {route,point:pathPointFromGeometry(routeGeometry,progress),phase:`на маршруте №${route.number} →`};
            }
            const progress=Math.max(0,Math.min(1,(nowMin-endMin)/Math.max(1,retMin-endMin)));
            return {route,point:returnGeometry?pathPointFromGeometry(returnGeometry,progress):[depot.lat,depot.lon],phase:'заезд в парк'};
        }
        ...
        function updateDepartureRouteInfo() {
            ...
            if (!el) return;
            if (!route) { el.textContent = 'Выбери маршрут — здесь появятся конечные и автоматический расчёт времени.'; return; }
            const duration = Number(route.calculatedDuration || 0);
            const distance = Number(route.calculatedDistance || 0);
            const terms = getRouteTerminals(route);
            const bits = [];
            if (duration) bits.push(`в пути: ${formatMinutesToDuration(duration)}`);
            if (distance) bits.push(`${(distance / 1000).toFixed(1)} км`);
            if (terms.length) bits.push(`конечных: ${terms.length}`);
            el.textContent = 'ℹ️ ' + (bits.length ? bits.join(' · ') : 'Для маршрута пока нет расчёта времени движения.') + '. Прибытие и заезд в парк рассчитаются автоматически.';
            const rows = document.querySelectorAll('#departureScheduleRows .departure-schedule-row');
            rows.forEach(row => populateDepartureTerminals(row, route));
        }
        ...
        function updateDepartureRouteForRows() {
            ...
            document.querySelectorAll('#departureScheduleRows .departure-schedule-row').forEach(row => {
                ...
                const routes = gameState.routes.filter(r => !vehicle || r.routeType === vehicle.category || (vehicle.category === 'bus' && r.routeType === 'bus'));
                ...
                if (routes.some(r => String(r.id) === String(prev))) select.value = prev;
                populateDepartureTerminals(row, routes.find(r => String(r.id) === String(select.value)) || routes[0]);
        ...
        function populateDepartureTerminals(row, route) {
            ...
            const prevStart = row.dataset.startStopId || startSel.value;
            const prevEnd = row.dataset.endStopId || endSel.value;
            ...
            if (terms.some(t => String(t.id) === String(prevStart))) startSel.value = prevStart; else if (terms[0]) startSel.value = terms[0].id;
            if (terms.some(t => String(t.id) === String(prevEnd))) endSel.value = prevEnd; else if (terms.length > 1) endSel.value = terms[terms.length-1].id;
            row.dataset.startStopId = startSel.value || '';
            row.dataset.endStopId = endSel.value || '';
        }

        function addDepartureScheduleRow(prefill) {
            const wrap = document.getElementById('departureScheduleRows');
            if (!wrap) return;
            const s = prefill || {day:'1',routeId:'',startStopId:'',endStopId:'',startTime:'07:30'};
            const row = document.createElement('div');
            row.className = 'departure-schedule-row departure-schedule-row-v2';
            row.innerHTML = `
                <select class="departure-day" aria-label="День недели">
                    <option value="1">Понедельник</option><option value="2">Вторник</option><option value="3">Среда</option><option value="4">Четверг</option><option value="5">Пятница</option><option value="6">Суббота</option><option value="0">Воскресенье</option>
                    <option value="weekdays">Все будни</option><option value="weekends">Все выходные</option><option value="all">Все дни</option>
                </select>
                ...
                <label class="departure-time-field"><span>Отправление с конечной</span><input class="departure-start" type="time" value="${escapeHtml(s.startTime || '07:30')}" required></label>
                <label class="departure-time-field"><span>Выезд из парка</span><input class="departure-depot" type="time" value="${escapeHtml(s.depotTime || '07:15')}" readonly></label>
                <label class="departure-time-field"><span>Прибытие на конечную</span><input class="departure-end" type="time" value="${escapeHtml(s.endTime || '08:30')}" readonly></label>
                <label class="departure-time-field"><span>Заезд в парк</span><input class="departure-return" type="time" value="${escapeHtml(s.returnTime || '08:45')}" readonly></label>
                <button type="button" class="btn-secondary departure-remove-row" onclick="this.closest('.departure-schedule-row').remove();" title="Удалить этот выезд">🗑️</button>`;
            wrap.appendChild(row);
            row.querySelector('.departure-day').value = String(s.day ?? '1');
            row.querySelector('.departure-route').addEventListener('change', () => { populateDepartureTerminals(row, getRouteTerminalsRoute(row)); autoCalculateDepartureRow(row); });
            row.querySelector('.departure-start-stop').addEventListener('change', () => { row.dataset.startStopId=row.querySelector('.departure-start-stop').value; autoCalculateDepartureRow(row); });
            row.querySelector('.departure-end-stop').addEventListener('change', () => { row.dataset.endStopId=row.querySelector('.departure-end-stop').value; autoCalculateDepartureRow(row); });
            row.querySelector('.departure-start').addEventListener('change', () => autoCalculateDepartureRow(row));
            updateDepartureRouteForRows();
        ...
        function collectDepartureSchedules() {
            ...
            document.querySelectorAll('#departureScheduleRows .departure-schedule-row').forEach(row => {
                const route = getRouteTerminalsRoute(row);
                const days = expandDepartureDay(row.querySelector('.departure-day')?.value || '1');
                if (!route) return;
                const schedule = {
                    routeId: route.id,
                    startStopId: row.querySelector('.departure-start-stop')?.value || route.terminalStopIds?.[0] || null,
                    endStopId: row.querySelector('.departure-end-stop')?.value || route.terminalStopIds?.[route.terminalStopIds.length-1] || null,
                    depotTime: row.querySelector('.departure-depot')?.value || '07:15',
                    startTime: row.querySelector('.departure-start')?.value || '07:30',
                    endTime: row.querySelector('.departure-end')?.value || '08:30',
                    returnTime: row.querySelector('.departure-return')?.value || '08:45'
                };
        ...
        function autoCalculateDepartureRow(row) {
            ...
            if (!start) return;
            const durationMin = Math.max(1, Math.round(Number(route.calculatedDuration || 900) / 60));
            const depotTransfer = 15;
            const startTime = start.value || '07:30';
            ...
            if (depot) depot.value = addMinutesToTime(startTime, -depotTransfer);
            if (end) end.value = addMinutesToTime(startTime, durationMin);
            if (ret) ret.value = addMinutesToTime(end?.value || startTime, depotTransfer);
            row.dataset.startStopId = row.querySelector('.departure-start-stop')?.value || '';
        ...
        function renderDepartureCards() {
            ...
            el.innerHTML = cards.map(card => {
                ...
                const rows = schedules.map(s => {
                    ...
                    return (s.days || []).map(day => `<div class="departure-schedule-view"><b>${DAY_FULL_RU[Number(day)] || 'День'}</b><span>🛣️ №${escapeHtml(r?.number || '—')}</span><span>🚏 ${escapeHtml(r?.start || '—')} → ${escapeHtml(r?.end || '—')}</span><span>🏁 ${escapeHtml(s.endTime)}</span><span>🏠 ${escapeHtml(s.returnTime)}</span></div>`).join('');
                }).join('');
                return `<div class="departure-card"><div class="departure-card-head"><div><b>🗓️ Карточка выезда · ${vehicleCategoryIcon(v.category)} ${escapeHtml(v.model)}</b><div class="interactive-muted">${escapeHtml(v.category === 'trolleybus' ? ('борт. №' + v.num) : (v.plate || v.num || 'без номера'))} · ${schedules.length} выездов/расписаний</div></div><button class="btn-secondary" onclick="toggleDepartureCard('${escapeHtml(card.id)}')">${card.active !== false ? '⏸ Остановить' : '▶ Запустить'}</button><button class="btn-secondary" onclick="deleteDepartureCard('${escapeHtml(card.id)}')">Удалить</button></div><div class="departure-schedule-list">${rows || '<div class="interactive-muted">Расписание не задано.</div>'}</div></div>`;
        ...
        function saveDepartureCard(event) {
            ...
            gameState.serviceCards.unshift({id,vehicleId:vehicle.id,routeId:firstRoute?.id || null,schedules,createdAt:new Date().toISOString(),lastArrivalCount:0,lastArrivalStamp:'',lastSimulationAt:new Date().toISOString(),active:true});
            ...
            addDepartureScheduleRow(); updateDepartureRouteOptions(); updateMapBusMarkers(true);
        }
        ...
        function getActiveDepartureSchedule(card, now = new Date()) {
            ...
            for (let i = 0; i < schedules.length; i++) {
                ...
                const depot = timeToMinutes(s.depotTime), ret = timeToMinutes(s.returnTime);
                const overnight = ret < depot;
                const active = overnight ? (minutesNow >= depot || minutesNow <= ret) : (minutesNow >= depot && minutesNow <= ret);
                if (active) {
                    let start = depot;
                    if (overnight && minutesNow <= ret) start -= 1440;
                    return {schedule:s,index:i,startMinutes:start,nowMinutes:minutesNow,minutesUntilStart:0};
                }
            }
            return null;
        }
        ...
        function renderDepartureSection() { updateDepartureRouteOptions(); renderDepartureCards(); const rows=document.getElementById('departureScheduleRows'); if(rows && !rows.children.length) addDepartureScheduleRow(); }
        ...
        function renderInteractive() {
            ...
            if (shopOwned) {
                shopOwned.innerHTML = gameState.owned.length
                    ? gameState.owned.map((v, i) => `<div class="owned-mini-card"><div><b>${vehicleCategoryIcon(v.category)} ${escapeHtml(v.model)}</b><div class="interactive-muted">${escapeHtml(v.category === 'trolleybus' ? ('борт. №' + v.num) : (v.plate || v.num || 'Без номера'))} · ${getVehicleRoute(v.id) ? 'маршрут №' + escapeHtml(getVehicleRoute(v.id).number) : 'не назначен'}</div></div><button class="btn-secondary" onclick="showGameSection('garage', document.querySelector('.game-menu-btn[onclick*=&quot;garage&quot;]'))">Гараж</button></div>`).join('')
