/* BUSPHOTO interactive bootstrap v109 — loads the real game split into part1/part2. */
(function(){
'use strict';
if(window.__BUSPHOTO_INTERACTIVE_BOOT_V109__) return;
window.__BUSPHOTO_INTERACTIVE_BOOT_V109__=true;
const VERSION='20260831-v109';
...
async function loadGameCore(){
 ...
 // Download both pieces at once, then execute them in their original order.
 // This removes the old visible loading wait while keeping part2 dependent on part1.
...
async function start(){
 // The old "Загрузка разделов…" badge is intentionally hidden.
 // The page should appear immediately; sections are loaded only when opened.
 status('');
 try{
  // Menu is the only HTML fragment needed for the first paint.
  await loadSection('menu');
  // Load the actual game core immediately. Both pieces are downloaded in parallel.
  await loadGameCore();
  ...
  // Do not block the first screen on Leaflet or auxiliary scripts.
  loadOptionalScripts();
  if(typeof window.ensureLeafletLoaded==='function') Promise.resolve().then(()=>window.ensureLeafletLoaded()).catch(()=>{});
  // Shop is visible first; load it after the game is already usable.
...
function boot(){
 if(window.__BUSPHOTO_INTERACTIVE_STARTED_V109__)return;
 window.__BUSPHOTO_INTERACTIVE_STARTED_V109__=true;
 start();
}
