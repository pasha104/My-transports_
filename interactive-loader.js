...
(function(){
  ...
  function refreshSectionData(section){try{
    if(section==='finance'){const el=document.getElementById('financeBalance'),owned=document.getElementById('financeOwned'),last=document.getElementById('financeLastPayout'); if(typeof gameState!=='undefined'){if(el)el.textContent=typeof money==='function'?money(gameState.balance):`${Math.round(Number(gameState.balance||0)).toLocaleString('ru-RU')} р.`;if(owned)owned.textContent=Array.isArray(gameState.owned)?gameState.owned.length:0;if(last)last.textContent=gameState.lastPayoutDate||'ещё не было';}}
  ...
  window.openInteractiveSection=async function(section,btn){try{
    ...
    window.__BUSPHOTO_PENDING_SECTION__={section,btn};await loadOne(section,{timeout:8000});if(typeof window.showGameSection==='function'){const p=window.__BUSPHOTO_PENDING_SECTION__;window.__BUSPHOTO_PENDING_SECTION__=null;window.showGameSection(p?.section||section,p?.btn||btn);refreshSectionData(p?.section||section);}
  ...
  async function loadMain(){if(document.querySelector('script[data-busphoto-interactive-main]'))return;await new Promise((resolve,reject)=>{const s=document.createElement('script');s.dataset.busphotoInteractiveMain='1';s.src='busphoto-interactive.js?v='+VERSION;s.onload=resolve;s.onerror=()=>reject(new Error('busphoto-interactive.js'));document.head.appendChild(s);});}
  async function start(){status('Запуск интерактива…');try{await loadOne('menu',{timeout:8000});}catch(e){console.error(e);status('Не удалось загрузить меню. Проверь соединение.',true);return;}try{await loadMain();}catch(e){console.error(e);status('Не удалось запустить интерактивный модуль.',true);return;}
    const initGame=()=>{if(typeof window.__BUSPHOTO_INIT_INTERACTIVE_GAME__!=='function'||window.__BUSPHOTO_GAME_INITIALIZED__)return;try{window.__BUSPHOTO_INIT_INTERACTIVE_GAME__();window.__BUSPHOTO_GAME_INITIALIZED__=true;console.info('[BUSPHOTO] game initialized');}catch(e){window.__BUSPHOTO_GAME_INITIALIZED__=false;console.error('[BUSPHOTO] game init failed',e?.stack||e);status('Интерактив запущен частично. Открой раздел ещё раз.',true);}};
    if(typeof window.ensureLeafletLoaded==='function')Promise.resolve().then(()=>window.ensureLeafletLoaded()).catch(()=>{});
    initGame();loadOne('shop',{timeout:8000}).then(()=>{if(!window.__BUSPHOTO_GAME_INITIALIZED__)initGame();}).catch(e=>console.warn('[BUSPHOTO] shop deferred',e));
    const st=document.getElementById('interactiveLoadingStatus');if(st)st.style.display='none';const p=window.__BUSPHOTO_PENDING_SECTION__;if(p&&typeof window.showGameSection==='function'){window.__BUSPHOTO_PENDING_SECTION__=null;window.showGameSection(p.section,p.btn);}
    const rest=['garage','finance','history','routes','rules'];let i=0;const next=()=>{if(i>=rest.length)return;const n=rest[i++];loadOne(n,{timeout:8000}).catch(e=>console.warn('[BUSPHOTO] background section',n,e));if('requestIdleCallback'in window)requestIdleCallback(next,{timeout:1500});else setTimeout(next,80);};next();
  }
