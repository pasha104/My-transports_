...
async function fetchText(url,timeout=8000){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),timeout);try{const r=await fetch(url,{cache:'no-store',credentials:'same-origin',signal:controller.signal});if(!r.ok)throw new Error(url+' HTTP '+r.status);return await r.text();}finally{clearTimeout(timer);}}
...
function loadScript(src,id){return new Promise((resolve,reject)=>{if(id&&document.getElementById(id)){resolve();return;}const s=document.createElement('script');if(id)s.id=id;s.src=src+'?v='+VERSION;s.async=false;s.onload=resolve;s.onerror=()=>reject(new Error('Не удалось загрузить '+src));document.head.appendChild(s);});}
window.openInteractiveSection=async function(name,button){try{await loadSection(name);if(typeof window.showGameSection==='function'){window.showGameSection(name,button);try{if(name==='garage'&&typeof renderGarageSection==='function')renderGarageSection();if(name==='history'&&typeof renderHistorySection==='function')renderHistorySection();if(name==='routes'&&typeof renderRoutes==='function')renderRoutes();if((name==='dispatch'||name==='stats'||name==='maintenance')&&typeof renderV43Panels==='function')renderV43Panels();}catch(e){console.warn('[BUSPHOTO] section refresh',e);}}else{throw new Error('Игровое ядро ещё не загружено');}}catch(e){console.error('[BUSPHOTO] section open',name,e);status('Интерактив ещё загружается. Нажми кнопку ещё раз.',true);}};
window.BUSPHOTOEnsureInteractiveSection=loadSection;
async function start(){status('');try{await loadSection('menu');await loadScript('busphoto-interactive.js','busphotoStableCore');if(typeof window.__BUSPHOTO_INIT_INTERACTIVE_GAME__==='function'&&!window.__BUSPHOTO_GAME_INITIALIZED__){window.__BUSPHOTO_INIT_INTERACTIVE_GAME__();window.__BUSPHOTO_GAME_INITIALIZED__=true;}loadScript('passenger-payout.js','busphotoPassengerPayoutScript').catch(()=>{});loadScript('card-map-preview.js','busphotoCardMapPreviewScript').catch(()=>{});if(typeof window.ensureLeafletLoaded==='function')Promise.resolve().then(()=>window.ensureLeafletLoaded()).catch(()=>{});loadSection('shop').catch(e=>console.warn('[BUSPHOTO] shop',e));}catch(e){console.error('[BUSPHOTO] bootstrap failed',e?.stack||e);status('Интерактив не удалось запустить. Обнови страницу.',true);}}
