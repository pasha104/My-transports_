/* BUSPHOTO — заработок от пассажиров v2 */
(function(){
'use strict';
if(window.__BUSPHOTO_PASSENGER_SYSTEM_V2__)return;
window.__BUSPHOTO_PASSENGER_SYSTEM_V2__=true;
...
const CAPACITY={
  ...
  trolleybus:{
    'МАЗ-103Т':108,'МАЗ-203Т':103,'МАЗ-303Т':90,'АКСМ-101':114,
    'БКМ-201':109,'БКМ-213':168,'БКМ-321':115,'БКМ-333':170,
...
function capacity(v){
  ...
  let best=0;
  for(const k of Object.keys(map)) if(name===k || name.startsWith(k)) best=Math.max(best,map[k]);
...
function vehicleMatches(v,event){
  ...
  if(!ev)return true;
  const nums=[v.num,v.boardNumber,v.garageNumber,v.plate,v.govNumber].map(x=>String(x??'').trim()).filter(Boolean);
...
function enrichEvent(event,state){
  if(!event || event.passengerSystemV2)return 0;
  ...
  event.total=total;
  event.passengerSystemV2=true;
  event.details=[`🕐 ${event.arrivalTime||event.time||'—'} · ТС ${v.model||'—'} · маршрут №${event.routeNumber||'—'} · 👥 ${p.count}/${p.capacity} пассажиров (${p.loadPercent}%) · 📏 ${distance.toFixed(1)} км · расстояние +${distancePayout} р. · пассажиры +${passengerPayout} р. · Итого +${total} р.`];
...
function install(){
  ...
  if(original.__passengerSystemV2)return true;
    ...
    for(const item of log){
      if(item.type==='route-arrival-batch'&&Array.isArray(item.arrivals)){
        ...
        for(const event of item.arrivals){
          ...
          if(d!==0||event.passengerSystemV2){batchDelta+=d;batchChanged=true;}
        }
        if(batchChanged){
          ...
          item.passengerSystemV2=true;
          item.details=item.arrivals.map(e=>e.details?.[0]||'');
          extra+=batchDelta;changed=true;
        }
        continue;
      }
      if(item.type!=='route-arrival'||item.passengerSystemV2)continue;
      const d=enrichEvent(item,after);
      if(d!==0||item.passengerSystemV2){extra+=d;changed=true;}
    }
    if(changed){
      ...
      if(typeof window.renderInteractiveHeaderAndLightViews==='function')try{window.renderInteractiveHeaderAndLightViews()}catch(e){}
      if(typeof window.renderInteractive==='function')try{window.renderInteractive()}catch(e){}
      if(typeof window.renderHistorySection==='function')try{window.renderHistorySection()}catch(e){}
    }
    return oldTotal+extra;
  }
  wrapped.__passengerSystemV2=true;
...
document.head.appendChild=function(node){
  if(node&&node.tagName==='SCRIPT'&&String(node.src||'').includes('busphoto-interactive.js')){
    const old=node.onload;
    node.onload=function(e){install();if(typeof old==='function')return old.call(this,e)};
  }
