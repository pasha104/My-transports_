...
(function(){
  ...
  function readGame(){try{const x=JSON.parse(localStorage.getItem(GAME_KEY)||'{}');return x&&typeof x==='object'?x:{};}catch(e){return {};}}
  ...
  function label(r){const n=r?.number??r?.routeNumber??'';const a=r?.start??r?.from??'—',b=r?.end??r?.to??'—';return '№'+n+' — '+a+' → '+b;}
  ...
  function removeAssignmentUI(){
    document.querySelectorAll('#routeAssignmentSummary .route-card').forEach(card=>{
      card.querySelectorAll('button').forEach(btn=>{if(/ТС\s+на\s+маршруте/i.test((btn.textContent||'').trim()))btn.remove();});
      card.querySelectorAll('.route-assignment').forEach(el=>el.remove());
      card.querySelectorAll('input[type="checkbox"]').forEach(el=>{const l=el.closest('label');if(l)l.remove();else el.remove();});
      [...card.querySelectorAll('*')].forEach(el=>{if(el.children.length)return;if(/^(?:⏸\s*)?(?:Без\s+ТС|\d+\s+ТС(?:\s+на\s+линии)?)$/i.test((el.textContent||'').trim()))el.remove();});
    });
  }

  function findRouteIdInCard(card){
    const direct=card.getAttribute?.('data-route-id')||card.dataset?.routeId;if(direct)return String(direct);
    const nodes=[card,...card.querySelectorAll('*')];
    for(const el of nodes){
      for(const attr of ['data-route-id','data-id']){const v=el.getAttribute?.(attr);if(v&&routes().some(r=>String(r.id)===String(v)))return String(v);}
      const oc=el.getAttribute?.('onclick')||'';
      const m=oc.match(/(?:routeId|route)\s*[:=,]\s*['"]?([^'"\),\s]+)/i);
      if(m&&routes().some(r=>String(r.id)===String(m[1])))return String(m[1]);
      const href=el.getAttribute?.('href')||'';
      const hm=href.match(/[?&]routeId=([^&#]+)/i);
      if(hm&&routes().some(r=>String(r.id)===decodeURIComponent(hm[1])))return decodeURIComponent(hm[1]);
    }
    const text=(card.textContent||'').replace(/\s+/g,' ');
    const hit=routes().find(r=>{const n=String(r.number??r.routeNumber??'').trim();return n&&text.includes('№'+n);});
    return hit?String(hit.id):null;
  }

  function addButtons(){
    const list=document.getElementById('routesList');if(!list)return;
    const rs=routes();if(!rs.length)return;
    const set=new Set([...[...list.children],...list.querySelectorAll('.route-card,.interactive-card,[data-route-id]')]);
    [...set].forEach(card=>{
      if(!(card instanceof HTMLElement)||card.querySelector('.bp-add-bus-btn'))return;
      const id=findRouteIdInCard(card);if(!id)return;
      const r=rs.find(x=>String(x.id)===id);if(!r||!isBusAllowed(r))return;
      const btn=document.createElement('button');btn.type='button';btn.className='bp-add-bus-btn';btn.textContent='🚌 Добавить автобус';
  ...
  async function ensureSection(){
    ...
    setTimeout(()=>{addButtons();removeAssignmentUI();},1600);
  }
